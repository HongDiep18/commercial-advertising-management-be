...
export class MailService {
  ...
  async sendAccountApprovedEmail(to: string, token: string): Promise<void> {
    ...
    const text = APPROVED_EMAIL.textBody(link);
    const html = APPROVED_EMAIL.htmlBody(link);
    try {
      if (this.transporter) {
      ...
      console.log(
        `[Mail] SMTP not configured – set-password link for ${to}: ${link}`,
      );
    } catch (err) {
      console.error('[Mail] sendAccountApprovedEmail failed:', err);
    }
  }
